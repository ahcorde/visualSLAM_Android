...
#define ccout if(mgvnBundleCout) cout

Eigen::Vector3d myBundleUnproject(Eigen::Vector2d& v)
{
...
Eigen::Vector4d myBundleUnproject(Eigen::Vector3d& v)
{
...
Eigen::Vector2d myBundleProject(Eigen::Vector3d& v)
{
...
inline void BundleTriangle_UpdateM6U_LL(Eigen::MatrixXd &m6U, Eigen::MatrixXd& m26A)
{
  for(int r=0; r<6; r++){
    for(int c=0; c<=r; c++){
...
inline void BundleTriangle_UpdateM3V_LL(Eigen::Matrix3d& m3V, Eigen::MatrixXd& m23B)
{
  for(int r=0; r<3; r++){
    for(int c=0; c<=r; c++){
...
int Bundle::AddCamera(mySE3 se3CamFromWorld, bool bFixed) {
  int n = mvCameras.size();
  Camera c;
  c.m6U = Eigen::MatrixXd(6, 6);//.resize(6, 6);
  c.v6EpsilonA = Eigen::VectorXd(6);
  c.bFixed = bFixed;
...
int Bundle::AddPoint(Eigen::Vector3d v3Pos) {
  int n = mvPoints.size();
  Point p;
  if(isnan(v3Pos.dot(v3Pos))){
//      std::cout << " You sucker, tried to give me a nan " << v3Pos << endl;
      v3Pos.setZero();
  }
...
void Bundle::AddMeas(int nCam, int nPoint, Eigen::Vector2d v2Pos, double dSigmaSquared)
{
...
void Bundle::ClearAccumulators() {
  for(size_t i=0; i<mvPoints.size(); ++i){
    mvPoints[i].m3V.setZero();
    mvPoints[i].v3EpsilonB.setZero();
  }
  for(size_t i=0; i<mvCameras.size(); ++i){
    mvCameras[i].m6U.setZero();
    mvCameras[i].v6EpsilonA.setZero();  
  }
}
...
int Bundle::Compute(bool *pbAbortSignal) {
  ...
  while(!mbConverged  && !mbHitMaxIterations && !*pbAbortSignal){
      ...
      if(gvsMEstimator == "Cauchy")
          ...
          ccout << "Invalid BundleMEstimator selected !! " << endl;
          ccout << "Defaulting to Tukey." << endl;
          gvsMEstimator = "Tukey";
          bNoError = Do_LM_Step<Tukey>(pbAbortSignal);
      }
  ...
  if(mbHitMaxIterations)
    ccout << "  Hit max iterations." << endl;

  ccout << "Final Sigma Squared: " << mdSigmaSquared << " (= " << sqrt(mdSigmaSquared) / 4.685 << " pixels.)" << endl;
  return mnAccepted;
}
...
inline void Bundle::ProjectAndFindSquaredError(Meas &meas) {
  ...
  meas.bBad = false;
  Eigen::Vector2d v2ImPlane = myBundleProject(meas.v3Cam);
  Eigen::Vector2d v2Image   = mCamera.Project(Eigen::Vector2d(v2ImPlane(0), v2ImPlane(1)));

  meas.m2CamDerivs = mCamera.GetProjectionDerivs_Eigen();
  meas.v2Epsilon = (meas.v2Found - v2Image)*meas.dSqrtInvNoise;
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal) {
  ...
  double gvdMinSigma = 0.4;//("Bundle.MinTukeySigma", 0.4, SILENT);
  ...
  for(list<Meas>::iterator itr = mMeasList.begin(); itr!=mMeasList.end(); itr++){
      ...
      // Re-weight error:
      meas.v2Epsilon =  meas.v2Epsilon*dWeight;
      ...
      Eigen::MatrixXd m2CamDerivs = dWeight * meas.m2CamDerivs;

      const double dOneOverCameraZ = 1.0 / meas.v3Cam(2);

      Eigen::Vector4d v4Cam = myBundleUnproject(meas.v3Cam);
      ...
      if(cam.bFixed) {
        ...
        for(int m=0;m<6;m++){
            Eigen::Vector4d v4Motion = mySE3::generator_field(m, v4Cam);
            ...
            meas.m26A.block(0,m,2,1) = meas.dSqrtInvNoise * m2CamDerivs * v2CamFrameMotion;
          }
      }
      
      // Calculate B: (the proj derivs WRT the point)
      for(int m=0;m<3;m++){
        Eigen::Vector3d v3Motion = cam.se3CfW.get_rotation().get_matrix().block(0,m,3,1);
        ...
        meas.m23B.block(0,m,2,1) = meas.dSqrtInvNoise * m2CamDerivs * v2CamFrameMotion;
      }
      ...
      if(!cam.bFixed){
          ...
          cam.v6EpsilonA += meas.m26A.transpose() * meas.v2Epsilon;
      ...
      point.v3EpsilonB += meas.m23B.transpose() * meas.v2Epsilon;
      
      if(cam.bFixed) {
        meas.m63W.setZero();
      } else {
        meas.m63W = meas.m26A.transpose() * meas.m23B;
      }
    }
  ...
  while(dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations && !*pbAbortSignal) {
    ...
    for(vector<Point>::iterator itr = mvPoints.begin(); itr!=mvPoints.end(); itr++) {
	    Point &point = *itr;
      Eigen::Matrix3d m3VStar = point.m3V;

      if(m3VStar(0, 0) * m3VStar(1,1) * m3VStar(2,2) == 0){
        point.m3VStarInv.setZero();
      } else {
        ...
        point.m3VStarInv = m3VStar.inverse();
      }
	  }
      ...
      Eigen::MatrixXd mS(mnCamsToUpdate * 6, mnCamsToUpdate * 6);
      mS.setZero();
      Eigen::MatrixXd vE(mnCamsToUpdate * 6, 1);
      vE.setZero();

      Eigen::MatrixXd m6(6,6); // Temp working space
      Eigen::MatrixXd v6(6,1); // Temp working space
      ...
      for(unsigned int j=0; j<mvCameras.size(); j++) {
        ...
        v6 = cam_j.v6EpsilonA;
          ...
          m6 -= pMeas->m63W * mvPoints[i].m3VStarInv * pMeas->m63W.transpose();  // SLOW SLOW should by 6x6sy
          v6 -= pMeas->m63W * (mvPoints[i].m3VStarInv * mvPoints[i].v3EpsilonB);
	      }

        mS.block(nCamJStartRow, nCamJStartRow, 6, 6) = m6;
        vE.block(nCamJStartRow, 0, 6, 1) = v6;
	    }
      ...
      for(unsigned int i=0; i<mvPoints.size(); i++)	{
	      ...
	      Meas* pMeas_ij;
	      Meas* pMeas_ik;
        Eigen::MatrixXd m63_MIJW_times_m3VStarInv(6, 3);
	    
	      for(vector<OffDiagScriptEntry>::iterator it=p.vOffDiagonalScript.begin(); it!=p.vOffDiagonalScript.end(); it++) {
          ...
          mS.block(nJRow, nKRow, 6, 6) -= m63_MIJW_times_m3VStarInv * pMeas_ik->m63W.transpose();
          assert(nKRow < nJRow);
        }
	    }
      ...
      //Got fat matrix S and vector E from part(iii). Now inverse the matrix, and find the camera update vector.
      Eigen::VectorXd vCamerasUpdate = mS.inverse()*vE;
      
      // Part (iv): Compute the map updates
      Eigen::VectorXd vMapUpdates(mvPoints.size() * 3);
      for(unsigned int i=0; i<mvPoints.size(); i++)	{
          Eigen::Vector3d v3Sum;
          v3Sum.setZero();
          for(unsigned int j=0; j<mvCameras.size(); j++){
              ...
              v3Sum+= pMeas->m63W.transpose() * vCamerasUpdate.segment(cam.nStartRow, 6);
          }

          Eigen::Vector3d v3 = mvPoints[i].v3EpsilonB - v3Sum;
          vMapUpdates.segment(i * 3, 3) = mvPoints[i].m3VStarInv * v3;

          if(isnan(vMapUpdates.segment(i * 3, 3).dot(vMapUpdates.segment(i * 3, 3)))){
            cerr << "NANNERY! " << endl;
      ...
      double dSumSquaredUpdate = vCamerasUpdate.dot(vCamerasUpdate) + vMapUpdates.dot(vMapUpdates);
      ...
      for(unsigned int j=0; j<mvCameras.size(); j++){
	      if(mvCameras[j].bFixed) {
	        ...
	        mvCameras[j].se3CfWNew = mySE3::exp(vCamerasUpdate.segment(mvCameras[j].nStartRow, 6)) * mvCameras[j].se3CfW;
        }
      }
      for(unsigned int i=0; i<mvPoints.size(); i++) {
        mvPoints[i].v3PosNew = mvPoints[i].v3Pos + vMapUpdates.segment(i * 3, 3);
      }
      // Calculate new error by re-projecting, doing tukey, etc etc:
      dNewError = FindNewError<MEstimator>();
      
      ccout <<setprecision(1) << "L" << mdLambda << setprecision(3) <<  "\tOld " << dCurrentError << "  New " << dNewError << "  Diff " << dCurrentError - dNewError << "\t";
      
      // Was the step good? If not, modify lambda and try again!!
      // (if it was good, will break from this loop.)
      if(dNewError > dCurrentError){
          ccout << " TRY AGAIN " << endl;
  ...
  ccout << "Nuked " << vit.size() << " measurements." << endl;
  return true;
}
...
double Bundle::FindNewError() {
  ...
  for(list<Meas>::iterator itr = mMeasList.begin(); itr!=mMeasList.end(); itr++) {
    Meas &meas = *itr;
    // Project the point.
    Eigen::Vector3d v3Cam = mvCameras[meas.c].se3CfWNew * mvPoints[meas.p].v3PosNew;
    ...
    Eigen::Vector2d v2ImPlane = myBundleProject(v3Cam);

    Eigen::Vector2d v2Image = mCamera.Project(v2ImPlane);

    Eigen::Vector2d v2Error = (meas.v2Found - v2Image)*meas.dSqrtInvNoise ;
    double dErrorSquared = v2Error.dot(v2Error);
...
Eigen::Vector3d Bundle::GetPoint(int n) {
  return mvPoints.at(n).v3Pos;
}

mySE3 Bundle::GetCamera(int n) {
