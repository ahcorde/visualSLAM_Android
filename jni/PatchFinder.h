...
public:
  ...
  // transformation.
  int CalcSearchLevelAndWarpMatrix(MapPoint &p, mySE3 se3CFromW, Eigen::Matrix2d &m2CamDerivs);
  ...
  void MakeTemplateCoarse(MapPoint &p, mySE3 se3CFromW, Eigen::Matrix2d &m2CamDerivs); // This also calculates the warp.
  void MakeTemplateCoarseNoWarp(MapPoint &p);  // Identity warp: just copies pixels from the source KF.
  void MakeTemplateCoarseNoWarp(KeyFrame &k, int nLevel, int irLevelPos0, int irLevelPos1); // Identity warp if no MapPoint struct exists yet.
  ...
  bool FindPatchCoarse(Eigen::Vector2d ir, KeyFrame &kf, unsigned int nRange);
  int ZMSSDAtPoint(cv::Mat &img, int icol, int irow); // This evaluates the score at one location

  // Results from step 3:
  // All positions are in the scale of level 0.
  inline Eigen::Vector2d GetCoarsePosAsVector() { return mv2CoarsePos; }
  ...
  inline Eigen::Vector2d GetSubPixPos()  { return mv2SubPixPos;   }  // Get result
  void SetSubPixPos(Eigen::Vector2d v2)  { mv2SubPixPos = v2;     }  // Set starting point
...
protected:
  ...
  cv::Mat mimTemplate;   // The matching template
  Eigen::MatrixXd mimJacs[2];  // Inverse composition jacobians; stored as floats to save a bit of space.
  ...
  Eigen::Vector2d mv2SubPixPos;     // In the scale of level 0
  double mdMeanDiff;          // Updated during inverse composition
  
  Eigen::Vector2d mv2CoarsePos;     // In the scale of level 0
  Eigen::Vector2d mirCenter;    // Quantized location of the center pixel of the NxN pixel template
  ...
  Eigen::MatrixXd mm2LastWarpMatrix;       // What was the last warp matrix this PatchFinder used?
};
