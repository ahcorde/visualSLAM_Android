...
Eigen::Vector2d myProject_mapMaker(Eigen::Vector3d& v)
{
...
Eigen::Vector3d myProject_mapMaker(Eigen::VectorXd& v)
{
    Eigen::Vector3d result;
    result(0) = v(0)/v(3);
    result(1) = v(1)/v(3);
...
void *thread_creation(void* v)
{
	MapMaker* base = (MapMaker*)v;
	base->run();
	return v;
...
MapMaker::MapMaker(Map& m, const ATANCamera &cam) : mMap(m), mCamera(cam)
{
  ...
  Reset();
  pthread_t thr_gui;
  //pthread_create(&thr_gui, NULL, &thread_creation, (void*) this);
  mgvdWiggleScale = 0.1;
}
...
void MapMaker::run()
{
  ...
  while(true) {
    CHECK_RESET;
//    msleep(5); // Sleep not really necessary, especially if mapmaker is busy
    CHECK_RESET;
...
void MapMaker::HandleBadPoints()
{
  ...
  for(unsigned int i=0; i<mMap.vpPoints.size(); i++){
	MapPoint* mpoint = mMap.vpPoints[i];
    if(mpoint->bBad){
        MapPoint *p = mMap.vpPoints[i];
...
Eigen::Vector3d MapMaker::ReprojectPoint(mySE3 se3AfromB, const Eigen::Vector2d &v2A, const Eigen::Vector2d &v2B) {
  Eigen::MatrixXd PDash_eigen;
  PDash_eigen.resize(3, 4);

  PDash_eigen.block(0, 0, 3, 3)  = se3AfromB.get_rotation().get_matrix();
  PDash_eigen(0, 3) = se3AfromB.get_translation()(0);
  PDash_eigen(1, 3) = se3AfromB.get_translation()(1);
  PDash_eigen(2, 3) = se3AfromB.get_translation()(2);

  Eigen::Matrix4d A_eigen;
  A_eigen(0, 0) = -1.0; A_eigen(0, 1) =  0.0; A_eigen(0, 2) = v2B(0); A_eigen(0, 3) = 0.0;
  A_eigen(1, 0) =  0.0; A_eigen(1, 1) = -1.0; A_eigen(1, 2) = v2B(1); A_eigen(1, 3) = 0.0;
  A_eigen.block(2, 0, 1, 4) = v2A(0) * PDash_eigen.block(2, 0, 1, 4) - PDash_eigen.block(0, 0, 1, 4);
  A_eigen.block(3, 0, 1, 4) = v2A(1) * PDash_eigen.block(2, 0, 1, 4) - PDash_eigen.block(1, 0, 1, 4);

  Eigen::JacobiSVD<Eigen::MatrixXd> svd_eigen(A_eigen, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::VectorXd v4Smallest_eigen = svd_eigen.matrixV().block(0,3,4,1);

  if(v4Smallest_eigen(3) == 0.0)
    v4Smallest_eigen(3) = 0.00001;

  Eigen::Vector3d psmall = myProject_mapMaker(v4Smallest_eigen);
...
bool MapMaker::InitFromStereo(KeyFrame &kF, KeyFrame &kS, vector<pair<Eigen::Vector2d, Eigen::Vector2d> > &vTrailMatches, mySE3& se3TrackerPose) {
  mdWiggleScale = mgvdWiggleScale; // Cache this for the new map.

  mCamera.SetImageSize(Eigen::Vector2d(kF.aLevels[0].im.cols, kF.aLevels[0].im.rows));
  
  vector<HomographyMatch> vMatches;
  for(unsigned int i=0; i<vTrailMatches.size(); i++){
      HomographyMatch m;
      Eigen::Vector2d v2d_first;
      Eigen::Vector2d v2d_second;
      Eigen::Matrix2d mjac;

      v2d_first = mCamera.UnProject(vTrailMatches[i].first);
      v2d_second = mCamera.UnProject(vTrailMatches[i].second);
      mjac = mCamera.GetProjectionDerivs_Eigen();

      m.v2CamPlaneFirst(0) = v2d_first(0);
      m.v2CamPlaneFirst(1) = v2d_first(1);
      m.v2CamPlaneSecond(0) = v2d_second(0);
      m.v2CamPlaneSecond(1) = v2d_second(1);
      m.m2PixelProjectionJac.resize(2,2);
      m.m2PixelProjectionJac = mjac;
...
//      cout << "  Could not init from stereo pair, try again." << endl;
  ...
  double dTransMagn = sqrt(se3.get_translation().dot(se3.get_translation()));
...
//      cout << "  Estimated zero baseline from stereo pair, try again." << endl;
  ...
  *pkSecond = kS;
  ...
  for(unsigned int i=0; i<vMatches.size(); i++){
      ...
      p->v3Center_NC = myUnproject(mCamera.UnProject(p->irCenter));
      ...
      Eigen::Vector2d aux3;
      aux3 = p->irCenter + aux1;
      Eigen::Vector2d aux4;
      aux4 = p->irCenter + aux2;

      p->v3OneRightFromCenter_NC = myUnproject(mCamera.UnProject( aux3 ));
      p->v3OneDownFromCenter_NC = myUnproject(mCamera.UnProject( aux4 ));

      p->v3Center_NC.normalize();
      p->v3OneDownFromCenter_NC.normalize();
      p->v3OneRightFromCenter_NC.normalize();
      ...
      finder.SetSubPixPos(vTrailMatches[i].second);
      ...
      // Triangulate point:
      Eigen::Vector2d v2SecondPos = finder.GetSubPixPos();

      Eigen::Vector2d point = mCamera.UnProject(v2SecondPos);

      p->v3WorldPos = ReprojectPoint(se3, point, vMatches[i].v2CamPlaneFirst);
      ...
      mFirst.v2RootPos = vTrailMatches[i].first;//vec(vTrailMatches[i].first);
      mFirst.bSubPix = true;
...
/// @ingroup gImage
Eigen::Vector2d rounded(const Eigen::Vector2d& v)
{
...
void MapMaker::ThinCandidates(KeyFrame &k, int nLevel)
{
  ...
  vector<Eigen::Vector2d> irBusyLevelPos;
  ...
  for(meas_it it = k.mMeasurements.begin(); it!=k.mMeasurements.end(); it++){
      if(!(it->second.nLevel == nLevel || it->second.nLevel == nLevel + 1))
        continue;
      irBusyLevelPos.push_back(rounded(it->second.v2RootPos/ LevelScale(nLevel)));
  }
  ...
  for(unsigned int i=0; i<vCSrc.size(); i++){
      Eigen::Vector2d irC = vCSrc[i].irLevelPos;
      bool bGood = true;
      for(unsigned int j=0; j<irBusyLevelPos.size(); j++){
          Eigen::Vector2d irB = irBusyLevelPos[j];
          if((irB - irC).squaredNorm() < nMinMagSquared){
              bGood = false;
              break;
          }
     }
...
void MapMaker::ApplyGlobalTransformationToMap(mySE3 se3NewFromOld)
{
...
bool isInsideBorder(cv::Mat img, int size, int px, int py)
{

  /*Check width*/
  if(px < size || px >  img.size().width - size - 1)
    return false;

  /*Check height*/
  if(py < size || py >  img.size().height - size - 1)
    return false;
...
bool MapMaker::AddPointEpipolar(KeyFrame &kSrc, KeyFrame &kTarget, int nLevel, int nCandidate) {
  ...
  static Eigen::MatrixXd imUnProj[2];
  static bool bMadeCache = false;

  if(!bMadeCache){
    imUnProj[0].resize(kSrc.aLevels[0].im.rows, kSrc.aLevels[0].im.cols);
    imUnProj[1].resize(kSrc.aLevels[0].im.rows, kSrc.aLevels[0].im.cols);
    for(int i=0;i<kSrc.aLevels[0].im.cols;i++)
      for(int j=0;j<kSrc.aLevels[0].im.rows;j++) { 
        Eigen::Vector2d p2d = mCamera.UnProject(Eigen::Vector2d(i,j)); 
        imUnProj[0](j,i) = p2d(0);
        imUnProj[1](j,i) = p2d(1);
      }   
    bMadeCache = true;
  }
  ...
  Eigen::Vector2d irLevelPos = candidate.irLevelPos;
  Eigen::Vector2d v2RootPos = LevelZeroPos(irLevelPos, nLevel);
  
  Eigen::Vector3d v3Ray_SC = myUnproject(mCamera.UnProject(v2RootPos));
  v3Ray_SC.normalize();

  Eigen::Vector3d v3LineDirn_TC = kTarget.se3CfromW.get_rotation() * (kSrc.se3CfromW.get_rotation().inverse() * v3Ray_SC);
  ...
  Eigen::Vector3d v3CamCenter_TC = kTarget.se3CfromW * kSrc.se3CfromW.inverse().get_translation(); // The camera end
  Eigen::Vector3d v3RayStart_TC = v3CamCenter_TC + dStartDepth * v3LineDirn_TC;                          // the far-away end
  Eigen::Vector3d v3RayEnd_TC = v3CamCenter_TC + dEndDepth * v3LineDirn_TC;                              // the far-away end
  ...
  Eigen::Vector2d v2A = myProject_mapMaker(v3RayStart_TC);
  Eigen::Vector2d v2B = myProject_mapMaker(v3RayEnd_TC);
  Eigen::Vector2d v2AlongProjectedLine = v2A-v2B;

  if(v2AlongProjectedLine.dot(v2AlongProjectedLine) < 0.00000001){
//    cout << "v2AlongProjectedLine too small." << endl;
    return false;
  }
  v2AlongProjectedLine.normalize();

  Eigen::Vector2d v2Normal;
  ...
  a = irLevelPos(0);
  b = irLevelPos(1);
  Finder.MakeTemplateCoarseNoWarp(kSrc, nLevel, a, b);
  if(Finder.TemplateBad())
    return false;
  
  vector<Eigen::Vector2d> &vv2Corners = kTarget.aLevels[nLevel].vImplaneCorners;
  vector<Eigen::Vector2d> &vIR = kTarget.aLevels[nLevel].vCorners;
  if(!kTarget.aLevels[nLevel].bImplaneCornersCached){
      for(unsigned int i=0; i<vIR.size(); i++) {  // over all corners in target img..
    	Eigen::Vector2d p  = vIR[i];
        Eigen::Vector2d zpos = LevelZeroPos(p, nLevel);
        Eigen::Vector2d vimg(imUnProj[0](zpos(1), zpos(0)), imUnProj[1](zpos(1), zpos(0))); 
        vv2Corners.push_back(vimg);
      }
  ...
  for(unsigned int i=0; i<vv2Corners.size(); i++) {  // over all corners in target img..
    Eigen::Vector2d v2Im = vv2Corners[i];
    ...
    int nZMSSD = Finder.ZMSSDAtPoint(kTarget.aLevels[nLevel].im, (int) vIR[i](0), (int) vIR[i](1));
  ...
  Finder.MakeSubPixTemplate();
  Eigen::Vector2d p = vIR[nBest];
  Finder.SetSubPixPos(LevelZeroPos(p, nLevel));
  bool bSubPixConverges = Finder.IterateSubPixToConvergence(kTarget,10);
  if(!bSubPixConverges)
    return false;
  
  // Now triangulate the 3d point...
  Eigen::Vector2d vtmp = Finder.GetSubPixPos();
  Eigen::Vector3d v3New = kTarget.se3CfromW.inverse() * ReprojectPoint(kSrc.se3CfromW * kTarget.se3CfromW.inverse(), mCamera.UnProject(v2RootPos), mCamera.UnProject(vtmp));
  ...
  pNew->v3Normal_NC(0) = 0;// = makeVector( 0,0,-1);
  pNew->v3Normal_NC(1) = 0;
  pNew->v3Normal_NC(2) = -1;
  pNew->irCenter = irLevelPos;
  pNew->rcolor = candidate.rColor;
  pNew->gcolor = candidate.gColor;
  pNew->bcolor = candidate.bColor;

  pNew->v3Center_NC = myUnproject(mCamera.UnProject(v2RootPos));
  ...
  pNew->v3OneRightFromCenter_NC = myUnproject(mCamera.UnProject( aux3 ));
  pNew->v3OneDownFromCenter_NC = myUnproject(mCamera.UnProject( aux4 ));

  pNew->v3Center_NC.normalize();
  pNew->v3OneDownFromCenter_NC.normalize();
  pNew->v3OneRightFromCenter_NC.normalize();
...
double MapMaker::KeyFrameLinearDist(KeyFrame &k1, KeyFrame &k2) {
  Eigen::Vector3d v3KF1_CamPos = k1.se3CfromW.inverse().get_translation();
  Eigen::Vector3d v3KF2_CamPos = k2.se3CfromW.inverse().get_translation();
  Eigen::Vector3d v3Diff = v3KF2_CamPos - v3KF1_CamPos;
  double dDist = sqrt(v3Diff.dot(v3Diff));
...
vector<KeyFrame*> MapMaker::NClosestKeyFrames(KeyFrame &k, unsigned int N)
{
  ...
  for(unsigned int i=0; i<mMap.vpKeyFrames.size(); i++){
      if(mMap.vpKeyFrames[i] == &k)
    	  continue;
      KeyFrame* keyf = mMap.vpKeyFrames[i];
      double dDist = KeyFrameLinearDist(k, *keyf);
      vKFandScores.push_back(make_pair(dDist, keyf));
  }
  ...
  for(unsigned int i=0; i<N; i++){
	  pair<double, KeyFrame* > p = vKFandScores[i];
...
bool MapMaker::NeedNewKeyFrame(KeyFrame &kCurrent)
{
  ...
  double MaxKFDistWiggleMult = 0.2; /*TODO: Default value was 1.0*/

  if(dDist > MaxKFDistWiggleMult * mdWiggleScaleDepthNormalized)
    return true;
  return false;
}
...
void MapMaker::BundleAdjustRecent()
{
  ...
  for(int i=0; i<4; i++){
	  KeyFrame*  keyf = vClosest[i];
	  if(keyf->bFixed == false){
		  sAdjustSet.insert(keyf);
	  }
  }
...
void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet, set<KeyFrame*> sFixedSet, set<MapPoint*> sMapPoints, bool bRecent)
{
  ...
  for(unsigned int i=0; i<mMap.vpKeyFrames.size(); i++){
	  KeyFrame* keyf = mMap.vpKeyFrames[i];
      if(mView_BundleID.count(keyf) == 0)
    	  continue;
      
      int nKF_BundleID = mView_BundleID[mMap.vpKeyFrames[i]];
      for(meas_it it= keyf->mMeasurements.begin();
	  it!= keyf->mMeasurements.end();
	  it++){
...
//	   << "   The map is probably corrupt: Ditching the map. " << endl;
  ...
  for(unsigned int i=0; i<vOutliers_PC_pair.size(); i++){
	  pair<int,int> pa = vOutliers_PC_pair[i];
      MapPoint *pp = mBundleID_Point[pa.first];
      KeyFrame *pk = mBundleID_View[pa.second];
...
bool MapMaker::ReFind_Common(KeyFrame &k, MapPoint &p)
{
  ...
  Eigen::Vector3d v3Cam_eigen = k.se3CfromW*p.v3WorldPos;


  if(v3Cam_eigen(2) < 0.001){
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }
  Eigen::Vector2d v2ImPlane = myProject_mapMaker(v3Cam_eigen);

  if(v2ImPlane.dot(v2ImPlane) > mCamera.LargestRadiusInImage() * mCamera.LargestRadiusInImage()){
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }
  
  Eigen::Vector2d v2Image = mCamera.Project(v2ImPlane);
  if(mCamera.Invalid()) {
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }

  if(v2Image[0] < 0 || v2Image[1] < 0 || v2Image[0] > k.aLevels[0].im.cols || v2Image[1] > k.aLevels[0].im.rows) {
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }
  
  Eigen::Matrix2d m2CamDerivs = mCamera.GetProjectionDerivs_Eigen();
  ...
  bool bFound = Finder.FindPatchCoarse(Eigen::Vector2d(v2Image[0], v2Image[1]), k, 4);  // Very tight search radius!
  if(!bFound) {
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }
...
int MapMaker::ReFindInSingleKeyFrame(KeyFrame &k)
{
  ...
  for(unsigned int i=0; i<vToFind.size(); i++){
	  MapPoint* m = vToFind[i];
	  if(ReFind_Common(k,*m))
		  nFoundNow++;
  }
...
// Find a dominant plane in the map, find an mySE3 to put it as the z=0 plane
mySE3 MapMaker::CalcPlaneAligner()
{
...
//      cout << "  MapMaker: CalcPlane: too few points to calc plane." << endl;
  ...
  int nRansacs = 100;
  Eigen::Vector3d v3BestMean_eigen;
  Eigen::Vector3d v3BestNormal_eigen;
  ...
  for(int i=0; i<nRansacs; i++){
      ...
      MapPoint* mapA = mMap.vpPoints[nA];
      MapPoint* mapB = mMap.vpPoints[nB];
      MapPoint* mapC = mMap.vpPoints[nC];

      Eigen::Vector3d v3Mean_eigen = 0.33333333 * (mapA->v3WorldPos +
                       mapB->v3WorldPos +
                       mapC->v3WorldPos);

      Eigen::Vector3d v3CA_eigen = mapC->v3WorldPos  - mapA->v3WorldPos;
      Eigen::Vector3d v3BA_eigen = mapB->v3WorldPos  - mapA->v3WorldPos;
      Eigen::Vector3d v3Normal_eigen = v3CA_eigen.cross(v3BA_eigen);

      if(v3Normal_eigen.dot( v3Normal_eigen)  == 0)
        continue;
      v3Normal_eigen.normalize();

      double dSumError = 0.0;
      for(unsigned int i=0; i<nPoints; i++){
          MapPoint* map = mMap.vpPoints[i];

          Eigen::Vector3d v3Diff_eigen = map->v3WorldPos - v3Mean_eigen;

          double dDistSq = v3Diff_eigen.dot(v3Diff_eigen);

          if(dDistSq == 0.0)
            continue;
          double dNormDist = fabs(v3Diff_eigen.dot(v3Normal_eigen));
     ...
     if(dSumError < dBestDistSquared){
        ...
        v3BestMean_eigen = v3Mean_eigen;
        v3BestNormal_eigen = v3Normal_eigen;
     }
  }
  
  // Done the ransacs, now collect the supposed inlier set
  vector<Eigen::Vector3d > vv3Inliers;
  for(unsigned int i=0; i<nPoints; i++){
      MapPoint* map = mMap.vpPoints[i];

      Eigen::Vector3d v3Diff_eigen = map->v3WorldPos - v3BestMean_eigen;
      double dDistSq = v3Diff_eigen.dot(v3Diff_eigen);
      if(dDistSq == 0.0)
        continue;
      double dNormDist = fabs(v3Diff_eigen.dot(v3BestNormal_eigen));

      if(dNormDist < 0.05){
          MapPoint* map = mMap.vpPoints[i];
          vv3Inliers.push_back(map->v3WorldPos);
      }
  }
  
  // With these inliers, calculate mean and cov
  Eigen::Vector3d v3MeanOfInliers = Eigen::Vector3d::Zero();
  for(unsigned int i=0; i<vv3Inliers.size(); i++){
      v3MeanOfInliers+=vv3Inliers[i];
  }
  v3MeanOfInliers *= (1.0 / vv3Inliers.size());

  Eigen::Matrix3d m3Cov_eigen = Eigen::Matrix3d::Zero();
  for(unsigned int i=0; i<vv3Inliers.size(); i++){
    Eigen::Vector3d v3Diff_eigen = vv3Inliers[i] - v3MeanOfInliers;
    m3Cov_eigen += v3Diff_eigen * v3Diff_eigen.transpose();
  }

  // Find the principal component with the minimal variance: this is the plane normal
  Eigen::EigenSolver<Eigen::MatrixXd> solver(m3Cov_eigen);
  Eigen::VectorXcd vcomplex = solver.eigenvectors().col(2);
  Eigen::Vector3d v3Normal_eigen;

  std::complex<double> a = vcomplex(0);
  std::complex<double> b = vcomplex(1);
  std::complex<double> c = vcomplex(2);

  v3Normal_eigen(0) = a.real();//real();
  v3Normal_eigen(1) = b.real();//.real();
  v3Normal_eigen(2) = c.real();//.real();

  
  // Use the version of the normal which points towards the cam center
  if(v3Normal_eigen(2) > 0)
    v3Normal_eigen *= -1.0;
  
  Eigen::Matrix3d m3Rot_eigen = Eigen::Matrix3d::Identity();
  m3Rot_eigen.block(2, 0 ,1, 3) = v3Normal_eigen.transpose();
  Eigen::Vector3d fila0 = m3Rot_eigen.block(0, 0 ,1, 3).transpose() ;
  Eigen::Vector3d aux = fila0 - v3Normal_eigen * ( fila0.dot(v3Normal_eigen) ) ;
  aux.normalize();
  fila0 = aux;
  m3Rot_eigen.block(0, 0 ,1, 3) = fila0.transpose();

  Eigen::Vector3d fila2 = m3Rot_eigen.block(2, 0 ,1, 3).transpose() ;
  m3Rot_eigen.block(1, 0 ,1, 3) = (fila2.cross(fila0)).transpose();

  mySE3 se3Aligner_eigen;
  se3Aligner_eigen.get_rotation() = m3Rot_eigen;
  Eigen::Vector3d v3RMean_eigen = se3Aligner_eigen * v3MeanOfInliers;
  se3Aligner_eigen.get_translation() = -v3RMean_eigen;
  
  return se3Aligner_eigen;
}
...
void MapMaker::RefreshSceneDepth(KeyFrame *pKF)
{
  ...
  for(meas_it it = pKF->mMeasurements.begin(); it!=pKF->mMeasurements.end(); it++){
      ...
      Eigen::Vector3d v3PosK = pKF->se3CfromW * point.v3WorldPos;
      dSumDepth += v3PosK(2);
